...
export default function StockAdjustments() {
  ...
  const columns = [
    { key: 'id', label: 'Adj #', render: (r) => <span className="font-mono text-xs font-semibold text-brand-700">{r.id}</span> },
    ...
    { key: 'reason', label: 'Reason', render: (r) => <span className="text-slate-600 max-w-xs truncate block">{r.reason}</span> },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Log and review all stock corrections and damages"
        action={<Button icon={Plus} onClick={() => setDrawerOpen(true)}>New Adjustment</Button>}
      />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title="Total Adjustments" value={String(counts.total)} icon={FileText} tone="brand" />
        <KpiCard title="Stock In" value={String(counts.in)} icon={ArrowDown} tone="emerald" />
        <KpiCard title="Stock Out" value={String(counts.out)} icon={ArrowUp} tone="rose" />
        <KpiCard title="Corrections" value={String(counts.adj)} icon={Shuffle} tone="amber" />
      </div>
