...
import { Ruler, Package, Boxes, Wallet } from 'lucide-react'
...
import { units as seedUnits } from '../data/products'
...
export default function InventoryUnits() {
  ...
  const rows = useMemo(() => {
    const names = new Set([...seedUnits, ...products.map((p) => p.unit).filter(Boolean)])
    return [...names]
      .map((name) => {
        const items = products.filter((p) => p.unit === name)
        return {
          id: name,
          name,
  ...
  const columns = [
    { key: 'name', label: t('units_label_short'), render: (r) => <span className="font-medium text-slate-900">{r.name}</span> },
  ...
  return (
    <div className="space-y-6">
      <PageHeader title={t('nav_units_of_measure')} subtitle={t('units_page_subtitle')} />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <KpiCard title={t('total_units_of_measure')} value={num(rows.length)} icon={Ruler} tone="brand" />
      ...
      <GroupedProductsDrawer
        ...
        title={`${t('products_in_unit')}: ${viewing?.name || ''}`}
