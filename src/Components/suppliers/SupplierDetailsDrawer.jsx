...
export default function SupplierDetailsDrawer({ open, onClose, supplier }) {
  ...
  return (
    <Drawer open={open} onClose={onClose} title={t('supplier_details')} size="lg">
      <div className="space-y-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <Badge tone={s.status === 'Active' ? 'success' : 'neutral'} dot>{t(`status_${s.status.toLowerCase()}`)}</Badge>
              <span className="text-xs text-slate-500">{t('since')} {formatDate(s.createdAt)}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
          ...
          <div className="flex items-center gap-2 text-slate-600"><CreditCard className="w-4 h-4 text-slate-400" /> {t('pan')}: {s.pan || '—'}</div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="p-4 rounded-lg bg-rose-50 border border-rose-100">
            <div className="text-xs text-rose-700">{t('payable')}</div>
            <div className="mt-1 text-lg font-bold text-rose-700">{formatCurrency(s.due)}</div>
          </div>
        </div>

        <div>
          <h4 className="text-sm font-semibold text-slate-900 mb-3">{t('purchase_ledger')}</h4>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200 text-xs text-slate-600 uppercase">
                    ...
                    <th className="text-left px-3 py-2 font-semibold">{t('method')}</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {s.transactions.map((tx) => (
                    <tr key={tx.id}>
                      ...
                      <td className="px-3 py-2"><Badge tone="info">{tx.method}</Badge></td>
